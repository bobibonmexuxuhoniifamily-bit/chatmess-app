...
class ChatMess {
    constructor() {
        ...
        this.swRegistration = null; // { uid, name }
        this.spamTracker = {};     // { uid: [timestamps] }
        this.spamMuted = {};       // { uid: untilTimestamp }
    ...
    async sendBroadcast() {
        ...
        // Push to globalNotifications — shows as banner on ALL clients
        await db.ref('globalNotifications').push({
            msg, time: Date.now(), from: 'admin'
        });

        document.getElementById('broadcastMsg').value = '';
        document.getElementById('broadcastRoom').value = '';
        this.toast('📢 Đã gửi thông báo tới tất cả!', 'success');
    }
    ...
    // ============================================
    listenNotifications() {
        // Only listen to notifications from the last 10s (avoid old ones on login)
        const since = Date.now() - 10000;
        db.ref('globalNotifications').orderByChild('time').startAt(since).on('child_added', snap => {
            const n = snap.val();
            if (!n || !n.msg) return;
            this.showNotifBanner(n.msg);
        });
    }

    showNotifBanner(text) {
        const banner = document.getElementById('adminNotifBanner');
        document.getElementById('adminNotifText').textContent = text;
        banner.classList.remove('hidden');
        banner.classList.add('banner-slide-in');
        // Auto-hide after 12s
        clearTimeout(this._bannerTimeout);
        this._bannerTimeout = setTimeout(() => this.hideNotifBanner(), 12000);
    }
    ...
    async startVoiceRecord() {
        ...
        try {
            ...
            this.mediaRecorder.onstop = () => {
                ...
                if (this.voiceSeconds < 1) { this.toast('⚠️ Giữ lâu hơn để ghi!', 'warning'); return; }
                if (blob.size > 3000000) { this.toast('⚠️ Âm thanh quá dài!', 'warning'); return; }
                const reader = new FileReader();
                reader.onload = ev => this.sendVoice(ev.target.result);
            ...
            document.getElementById('voiceTimer').textContent = '0s';
            this.voiceTimerInterval = setInterval(() => {
                this.voiceSeconds++;
                document.getElementById('voiceTimer').textContent = this.voiceSeconds + 's';
                if (this.voiceSeconds >= 60) this.stopVoiceRecord(); // max 60s
            }, 1000);
        } catch(e) {
            this.toast('❌ Không thể dùng mic: ' + e.message, 'error');
        }
    }
    ...
    cancelVoiceRecord() {
        ...
        document.getElementById('voiceRecordingBar').classList.add('hidden');
        document.getElementById('voiceBtn').classList.remove('recording');
        this.toast('🚫 Đã huỷ ghi âm', 'info');
    }

    sendVoice(dataUrl) {
        const msg = {
            text: '', name: this.user.displayName, userId: this.user.uid,
            color: this.user.color, time: firebase.database.ServerValue.TIMESTAMP,
            type: 'voice', audioData: dataUrl,
            duration: this.voiceSeconds
        };
        ...
        this.toast('🎙️ Đã gửi tin nhắn thoại!', 'success');
    }
    ...
    checkSpam() {
        ...
        // Check mute
        if (this.spamMuted[uid] && now < this.spamMuted[uid]) {
            const remaining = Math.ceil((this.spamMuted[uid] - now) / 1000);
            this.toast('⏳ Bạn đang bị tạm khóa vì spam! Còn ' + remaining + 's', 'warning');
            return false;
        }

        // Track timestamps
        if (!this.spamTracker[uid]) this.spamTracker[uid] = [];
        this.spamTracker[uid].push(now);
        // Keep only last 3 seconds
        this.spamTracker[uid] = this.spamTracker[uid].filter(t => now - t < 3000);

        if (this.spamTracker[uid].length >= 6) {
            this.spamMuted[uid] = now + 15000; // mute 15 seconds
            this.spamTracker[uid] = [];
            this.toast('🚫 Gửi quá nhanh! Tạm khóa 15 giây.', 'error');
            return false;
        }
        return true;
    }
    ...
    // ============================================
    async autoCleanupMessages(path, userId) {
        try {
            const snap = await db.ref(path).orderByChild('time').once('value');
            const userMsgs = [];
            snap.forEach(c => {
                const v = c.val();
                if (v.userId === userId) userMsgs.push({ key: c.key, time: v.time });
            });
            if (userMsgs.length > 30) {
                const toDelete = userMsgs.slice(0, userMsgs.length - 15); // keep newest 15
                const updates = {};
                toDelete.forEach(m => { updates[m.key] = null; });
    ...
    async initNotifications() {
        ...
        if (Notification.permission === 'granted') {
            ...
            this._updateNotifBtn(true);
        } else {
            this._updateNotifBtn(false);
        }
    }

    async toggleNotifications() {
        if (!('Notification' in window)) { this.toast('⚠️ Trình duyệt không hỗ trợ thông báo!', 'warning'); return; }
        if (this.notifGranted) {
            this.notifGranted = false;
            this._updateNotifBtn(false);
            this.toast('🔕 Đã tắt thông báo', 'info');
            return;
        }
        if (Notification.permission === 'denied') { this.toast('🚫 Bị chặn! Vào cài đặt trình duyệt để bật lại.', 'warning'); return; }
        const result = await Notification.requestPermission();
        if (result === 'granted') {
            this.notifGranted = true;
            this._updateNotifBtn(true);
            this.toast('🔔 Đã bật thông báo!', 'success');
            setTimeout(() => this.pushNotif('🔔 ChatMess 2.5', 'Thông báo đã bật thành công!', null), 600);
        } else {
            this.toast('❌ Không được cấp quyền!', 'error');
        }
    }
    ...
    _updateNotifBtn(on) {
        ...
        btn.title = on ? 'Tắt thông báo' : 'Bật thông báo';
        btn.style.background = on
            ? 'linear-gradient(135deg,#7C3AED,#06B6D4)'
            : 'linear-gradient(135deg,#7C3AED,#9F67F8)';
    }

    pushNotif(title, body, iconUrl) {
        if (!this.notifGranted) return;
        if (document.hasFocus() && document.visibilityState === 'visible') return;
        const opts = { body: body || '', icon: iconUrl || null, tag: 'chatmess-' + Date.now(), renotify: true, vibrate: [200,100,200] };
        if (this.swRegistration?.active) {
            this.swRegistration.active.postMessage({ type:'SHOW_NOTIF', title, body: opts.body, icon: opts.icon, tag: opts.tag });
        } else {
    ...
    renderMsg(msg) {
        ...
        if (msg.censored) {
            ...
            if (msg.type === 'voice' && msg.audioData) content += `<div class="msg-voice-wrap"><i class="fas fa-microphone-alt voice-icon"></i><audio class="msg-audio" controls src="${msg.audioData}"></audio>${msg.duration ? '<span class="voice-dur">'+msg.duration+'s</span>' : ''}</div>`;
        ...
        if (!mine) {
            const rLabel = this.chatMode === 'dm'
                ? (this.allUsers[this.currentDM]?.displayName || 'DM')
                : (this.allRooms[this.currentRoom]?.info?.name || this.currentRoom || 'Chat');
            const nBody = msg.type==='voice' ? '🎙️ Tin nhắn thoại'
                : msg.type==='image' ? '📷 Đã gửi ảnh'
                : msg.type==='video' ? '🎬 Đã gửi video'
                : (msg.text||'').slice(0,100);
            const av = Object.values(this.allUsers||{}).find(u=>u.username===msg.name)?.avatarData||null;
            this.pushNotif(`${msg.name} • ${rLabel}`, nBody, av);
        }
    }
    ...
    sendMessage() {
        ...
        const path = this.chatMode === 'dm'
            ...
            : 'rooms/' + this.currentRoom + '/messages';
        db.ref(path).push(msg);
        // 🧹 Auto-cleanup if too many messages from this user
        this.autoCleanupMessages(path, this.user.uid);
        input.value = '';
    ...
    bindEvents() {
        ...
        document.getElementById('notifBtn')?.addEventListener('click', () => this.toggleNotifications());
        // Admin panel
        ...
        voiceBtn.addEventListener('mousedown', () => this.startVoiceRecord());
        voiceBtn.addEventListener('mouseup', () => this.stopVoiceRecord());
        voiceBtn.addEventListener('mouseleave', () => { if (this.isRecording) this.stopVoiceRecord(); });
        voiceBtn.addEventListener('touchstart', e => { e.preventDefault(); this.startVoiceRecord(); });
        voiceBtn.addEventListener('touchend', e => { e.preventDefault(); this.stopVoiceRecord(); });
        document.getElementById('voiceCancelBtn').addEventListener('click', () => this.cancelVoiceRecord());

        // Admin notification banner close
        document.getElementById('adminNotifClose').addEventListener('click', () => this.hideNotifBanner());
